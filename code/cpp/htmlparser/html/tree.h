...
public:
    using tree_node_ptr_t = tree_node_<T>*;
    tree_node_<T>* parent;
    ...
    std::vector<tree_node_ptr_t> children() const {
        auto* child_node = first_child;
        if ( child_node == nullptr) return std::vector<tree_node_ptr_t>();

        std::vector<tree_node_ptr_t> child_nodes;
    ...
    std::vector<const tree_node_ptr_t> children_const() const {
        auto* child_node = first_child;
        if ( child_node == nullptr) return std::vector<const tree_node_ptr_t>();

        std::vector<const tree_node_ptr_t> child_nodes;
        child_nodes.push_back(child_node);
    ...
    int32_t indexOfChild(const tree_node_<T>* childNode) const
    {
        auto* n = this->first_child;
        int32_t index = -1;
        while (n != nullptr) {
            ++index;
            if (n == childNode) {
    ...
    tree_node_ptr_t childAt(const int32_t index) const
    {
        auto* n = this->first_child;
        int32_t i = -1;
        while (n != nullptr) {
