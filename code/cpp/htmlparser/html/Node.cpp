...
void Node::parseAttributes() 
{
	...
	while (*ptr && *ptr != '>') 
	{
        ...
        key.assign(static_cast<string::size_type>(end - ptr), '\0');
...
//            fprintf(stderr, "FIXMENM '%s' = '%s'\n", key.c_str(), val.c_str());
            attributeAppend(key, val);
		}
		else
		{
//            fprintf(stderr, "FIXMENM D: '%s'\n", key.c_str());
			if (!key.empty())
			{
                attributeAppend(key, string());
			}
		}
    }
}

void Node::sortAttributes()
{
    auto fnSort = [&](const Attribute& l, const Attribute& r) -> bool { return l.name < r.name; };
    std::sort(m_Attributes.begin(), m_Attributes.end(), fnSort);
}
