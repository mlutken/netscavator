#ifndef _WEBBROWSE_DOM_EVENT_TARGET_H_
#define _WEBBROWSE_DOM_EVENT_TARGET_H_
...
@status FROZEN */
class DOMEventTarget : public Supports
{
	MOZ_CONSTRUCT(DOMEventTarget,Supports);
public:
...
//	AddEventListener ( const nsAString & type, nsIDOMEventListener *listener, PRBool useCapture ) = 0;
...
//	RemoveEventListener ( const nsAString & type, nsIDOMEventListener *listener, PRBool useCapture ) = 0;
	 ...
	 *              exception.
	 */
	bool	dispatchEvent ( boost::shared_ptr<DOMEvent> pEvent	){
		PRBool bRetVal;
		ifMoz()->DispatchEvent ( pEvent->ifMoz(), &bRetVal );
		return static_cast<bool>( bRetVal != 0 );													\
	}
};

#endif // _WEBBROWSE_DOM_EVENT_TARGET_H_
