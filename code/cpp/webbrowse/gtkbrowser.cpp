...
//#include <gtkembedmoz/gtkmozembed_glue.cpp>

// // #include <gtkembedmoz/gtkmozembed.h>
// // #include <gtkembedmoz/gtkmozembed_internal.h>
// // #include <xpcom/nsXPCOMGlue.h>
...
#include "MozBrowser_examples.cpp"
...
//	g_print ( "1: load_finished_cb: %d\n", iLoadCount );
	browser->m_pMozBrowser->signalsManager()->signalPageLoaded(*(browser->m_pMozBrowser));
//	g_print ( "2: load_finished_cb: %d\n", iLoadCount );
...
// 		if ( webNavigation ) {
// 			printf ("\nJuhu\n\n");
// 			nsresult rv = webNavigation->LoadURI(NS_ConvertASCIItoUTF16("http://www.home.dk").get(),nsIWebNavigation::LOAD_FLAGS_NONE, 0, 0, 0);
...
// 	gtk_moz_embed_get_nsIWebBrowser ( GTK_MOZ_EMBED ( newBrowser->mozEmbed ), getter_AddRefs ( pIWebBrowser ) );
// 	boost::shared_ptr<WebBrowser> pWebBrowser( new WebBrowser(pIWebBrowser) );
// 	newBrowser->m_pMozBrowser = boost::shared_ptr<MozBrowser>(new MozBrowser(pWebBrowser) ) ;
...
{
//	g_print ( "QQQQQQ open_uri_cb %s\n", uri );
	bool bOkToLoad = true;
//	bOkToLoad = browser->m_pMozBrowser->signalsManager()->signalOnLoadUri(*(browser->m_pMozBrowser), uri);
	browser->m_pMozBrowser->signalsManager()->signalOnLoadUri(*(browser->m_pMozBrowser), uri);
	// interrupt this test load
	if ( !strcmp ( uri, "http://www.home.dk/flashSmall.jsp?url=http://streaming.home.dk/Film_klikenpris2008/home_164x125.swf&height=125&width=164" ) ) {
		return TRUE;
	}
...
//	g_print("dom_key_press_cb\n");
	...
	boost::shared_ptr<DOMWindow>	pDOMWindow = browser->m_pMozBrowser->webBrowser()->contentDOMWindow();
//	nsCOMPtr<nsIWebBrowser> webBrowser;
//	gtk_moz_embed_get_nsIWebBrowser ( GTK_MOZ_EMBED ( embed ), getter_AddRefs ( webBrowser ) );
	
//	ex_HomeSearch ( browser->m_pMozBrowser );
//	ex_DomTest ( browser->m_pMozBrowser );
	
	boost::shared_ptr<DOMKeyEvent> pKeyEvent(new DOMKeyEvent(event) );
	
	browser->m_pMozBrowser->signalsManager()->signalKeyPress(*(browser->m_pMozBrowser), pKeyEvent );
	return NS_OK;
}
                          ...
                          TestGtkBrowser *browser )
{
	...
	boost::shared_ptr<DOMMouseEvent> pMouseEvent(new DOMMouseEvent(event) );
	
	browser->m_pMozBrowser->signalsManager()->signalMouseClick( *(browser->m_pMozBrowser), pMouseEvent );
		
//	cout << b::format("Std NAME: %s") % pDOMWindow->name() << endl ;	 
//	ex_AllElements ( browser->m_pMozBrowser ) ;
//	ex_HomeSearch ( browser->m_pMozBrowser );
//	ex_Click( browser->m_pMozBrowser );
//	ex_Links ( browser->m_pMozBrowser );
//	ex_HTMLElements ( browser->m_pMozBrowser );
//	ex_DOMWindow_Name ( pDOMWindow );	
//	ex_DOM ( browser->m_pMozBrowser );
//	ex_Elements ( browser->m_pMozBrowser );
//	ex_WebNavigation ( browser->m_pMozBrowser );
//	ex_Javascript ( browser->m_pMozBrowser );
	
	
	//ex_DOMNodeVec ( browser->m_pMozBrowser );
	//ex_FormGet ( domDocument );
	//ex_nsCache();
//	ex_javascript1(webBrowser);
