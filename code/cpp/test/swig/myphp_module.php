...
if (!extension_loaded("myphp_module")) {
  if (strtolower(substr(PHP_OS, 0, 3)) === 'win') {
    if (!dl('php_myphp_module.dll')) return;
  } else {
    ...
    if (PHP_SHLIB_SUFFIX === 'PHP_SHLIB_SUFFIX' || PHP_SHLIB_SUFFIX === 'dylib') {
      if (!dl('myphp_module.so')) return;
    } else {
      if (!dl('myphp_module.'.PHP_SHLIB_SUFFIX)) return;
    }
  }
}
...
class Vector {
	...
	function __construct($iX,$iY,$iZ) {
		$this->_cPtr=new_Vector($iX,$iY,$iZ);
	}

	function len2() {
		return Vector_len2($this->_cPtr);
	}
}
