...
static void gtk_moz_embed_download_class_init(MinGtkMozEmbedDownloadClass *klass);
static void gtk_moz_embed_download_init(MinGtkMozEmbedDownload *embed);
...
gtk_moz_embed_download_get_type(void)
{
  ...
  if (!moz_embed_download_type)
  {
    ...
    {
      "MinGtkMozEmbedDownload",
      sizeof(MinGtkMozEmbedDownload),
      sizeof(MinGtkMozEmbedDownloadClass),
...
static void
gtk_moz_embed_download_class_init(MinGtkMozEmbedDownloadClass *klass)
{
  ...
  moz_embed_download_signals[DOWNLOAD_STARTED_SIGNAL] =
    gtk_signal_new("started",
                   ...
                   GTK_SIGNAL_OFFSET(MinGtkMozEmbedDownloadClass, started),
  ...
  moz_embed_download_signals[DOWNLOAD_COMPLETED_SIGNAL] =
    gtk_signal_new("completed",
                   GTK_RUN_FIRST,
                   GET_OBJECT_CLASS_TYPE(klass),
                   GTK_SIGNAL_OFFSET(MinGtkMozEmbedDownloadClass, completed),
  ...
  moz_embed_download_signals[DOWNLOAD_FAILED_SIGNAL] =
    gtk_signal_new("error",
                   GTK_RUN_FIRST,
                   GET_OBJECT_CLASS_TYPE(klass),
                   GTK_SIGNAL_OFFSET(MinGtkMozEmbedDownloadClass, error),
                   gtk_marshal_NONE__NONE,
                   GTK_TYPE_NONE, 0);
  ...
  moz_embed_download_signals[DOWNLOAD_DESTROYED_SIGNAL] =
    gtk_signal_new("aborted",
                   GTK_RUN_FIRST,
                   GET_OBJECT_CLASS_TYPE(klass),
                   GTK_SIGNAL_OFFSET(MinGtkMozEmbedDownloadClass, aborted),
                   gtk_marshal_NONE__NONE,
                   GTK_TYPE_NONE, 0);
  ...
  moz_embed_download_signals[DOWNLOAD_PROGRESS_SIGNAL] =
    gtk_signal_new("progress",
                   GTK_RUN_FIRST,
                   GET_OBJECT_CLASS_TYPE(klass),
                   GTK_SIGNAL_OFFSET(MinGtkMozEmbedDownloadClass, progress),
...
static void
gtk_moz_embed_download_init(MinGtkMozEmbedDownload *download)
{
...
gtk_moz_embed_download_destroy(GtkObject *object)
{
  ...
  g_return_if_fail(MINGTK_IS_MOZ_EMBED_DOWNLOAD(object));

  MinGtkMozEmbedDownload  *embed;
  EmbedDownload *downloadPrivate;

  embed = MINGTK_MOZ_EMBED_DOWNLOAD(object);
...
void
gtk_moz_embed_download_do_command(MinGtkMozEmbedDownload *item, guint command)
{
  ...
  if (command == MINGTK_MOZ_EMBED_DOWNLOAD_CANCEL) {
    download_priv->launcher->Cancel(MINGTK_MOZ_EMBED_STATUS_FAILED_USERCANCELED);
    download_priv->launcher->SetWebProgressListener(nsnull);

    return;
  }

  if (command == MINGTK_MOZ_EMBED_DOWNLOAD_RESUME) {
  ...
  if (command == MINGTK_MOZ_EMBED_DOWNLOAD_PAUSE) {
  ...
  if (command == MINGTK_MOZ_EMBED_DOWNLOAD_RELOAD) {
    if (download_priv->gtkMozEmbedParentWidget) {}
  }
  // FIXME: missing MINGTK_MOZ_EMBED_DOWNLOAD_STORE and MINGTK_MOZ_EMBED_DOWNLOAD_RESTORE implementation.
}

gchar*
gtk_moz_embed_download_get_file_name(MinGtkMozEmbedDownload *item)
{
  EmbedDownload *download_priv = (EmbedDownload *) item->data;
...
gchar*
gtk_moz_embed_download_get_url(MinGtkMozEmbedDownload *item)
{
  EmbedDownload *download_priv = (EmbedDownload *) item->data;
...
glong
gtk_moz_embed_download_get_progress(MinGtkMozEmbedDownload *item)
{
  EmbedDownload *download_priv = (EmbedDownload *) item->data;
...
glong
gtk_moz_embed_download_get_file_size(MinGtkMozEmbedDownload *item)
{
  EmbedDownload *download_priv = (EmbedDownload *) item->data;
