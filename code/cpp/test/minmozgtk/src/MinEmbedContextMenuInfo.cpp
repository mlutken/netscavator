 ...
 *
 * ***** END LICENSE BLOCK ***** */
#include "MinEmbedContextMenuInfo.h"
  ...
  if (targetContent && targetContent->IsNodeOfType(nsIContent::eHTML_FORM_CONTROL)) {
    ...
    if (formControl) {
      ...
      case NS_FORM_INPUT_FILE:
        mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_INPUT;
        break;
      ...
      case NS_FORM_INPUT_PASSWORD:
        mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_INPUT;
        mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_IPASSWORD;
        break;
      ...
      if (mEmbedCtxType & MINGTK_MOZ_EMBED_CTX_INPUT) {
        ...
        if (mCtxFormType == NS_FORM_TEXTAREA) {
          ...
          if (!NS_FAILED(rv) && rdonly) {
            mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_ROINPUT;
          }
        } else {
...
EmbedContextMenuInfo::CheckDomHtmlNode(nsIDOMNode *aNode)
{
  ...
  if (uTag.LowerCaseEqualsLiteral("object")) {
    ...
    mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_LINK;
    if (anchor && !mCtxHref.IsEmpty()) {
      if (mCtxHref.LowerCaseEqualsLiteral("text/smartbookmark")) {
        ...
        if (childNode) {
          ...
          if (NS_SUCCEEDED(rv))
            mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_IMAGE;
        }
      } else if (StringBeginsWith(mCtxHref, NS_LITERAL_STRING("mailto:"))) {
        mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_EMAIL;
      }
    }
  }
...
EmbedContextMenuInfo::UpdateContextData(nsIDOMEvent *aDOMEvent)
{
  ...
  //    nsresult SelText = mOwner->ClipBoardAction(MINGTK_MOZ_EMBED_CAN_COPY);
  if (originalNode == mOrigNode)
    return NS_OK;

  mEmbedCtxType = MINGTK_MOZ_EMBED_CTX_NONE;
  ...
  if (mOrigNode) {
    ...
    if (SOrigNode.EqualsLiteral("xul:thumb")
      ...
      mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_XUL;
      return NS_OK;
    }
  }
  if (mCtxEvent)
  ...
  if (NS_SUCCEEDED(rv) && domDoc && mCtxDocument != domDoc) {
    ...
    if (mainDocument != domDoc) {
      mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_IFRAME;
      SetFrameIndex();
    }
  }
  nsCOMPtr<nsIDOMElement> targetDOMElement;
  ...
  if (htmlDoc) {
    ...
    if (DMode.EqualsLiteral("on")) {
      mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_INPUT;
      mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_RICHEDIT;
    }
  }
  nsCOMPtr<nsIDocument> doc = do_QueryInterface(mCtxDocument);
  if (!doc)
  ...
  nsIFrame* frame = nsnull;
#if defined(FIXED_BUG347731) || !defined(MOZ_ENABLE_LIBXUL)
  if (mEmbedCtxType & MINGTK_MOZ_EMBED_CTX_RICHEDIT)
  ...
  mEmbedCtxType |= MINGTK_MOZ_EMBED_CTX_DOCUMENT;
  return NS_OK;
}
