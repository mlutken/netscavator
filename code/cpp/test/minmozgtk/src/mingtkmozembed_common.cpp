...
#include "mingtkmozembed_common.h"
...
#include "MinEmbedPasswordMgr.h"
#include "nsIPassword.h"
#endif

#include "MinEmbedGlobalHistory.h"
...
static void
gtk_moz_embed_common_class_init(MinGtkMozEmbedCommonClass *klass);

static void
gtk_moz_embed_common_init(MinGtkMozEmbedCommon *embed);
...
gtk_moz_embed_common_get_type(void)
{
  ...
  if (!moz_embed_common_type)
  {
    ...
    {
      "MinGtkMozEmbedCommon",
      sizeof(MinGtkMozEmbedCommon),
      sizeof(MinGtkMozEmbedCommonClass),
...
gtk_moz_embed_common_class_init(MinGtkMozEmbedCommonClass *klass)
{
  ...
  moz_embed_common_signals[COMMON_CERT_ERROR] =
    gtk_signal_new("certificate-error",
                   ...
                   GTK_SIGNAL_OFFSET(MinGtkMozEmbedCommonClass,
                                     certificate_error),
  ...
  moz_embed_common_signals[COMMON_ASK_COOKIE] =
    gtk_signal_new("ask-cookie",
                   GTK_RUN_FIRST,
                   GET_OBJECT_CLASS_TYPE(klass),
                   GTK_SIGNAL_OFFSET(MinGtkMozEmbedCommonClass, ask_cookie),
...
static void
gtk_moz_embed_common_init(MinGtkMozEmbedCommon *common)
{
...
gtk_moz_embed_common_destroy(GtkObject *object)
{
  ...
  g_return_if_fail(MINGTK_IS_MOZ_EMBED_COMMON(object));
  MinGtkMozEmbedCommon  *embed = nsnull;
  EmbedCommon *commonPrivate = nsnull;
  embed = MINGTK_MOZ_EMBED_COMMON(object);
...
{
  MinGtkMozEmbedSecurityMode sec_mode;
  ...
  switch (sec_state & wpl_security_bits) {
    case nsIWebProgressListener::STATE_IS_INSECURE:
      sec_mode = MINGTK_MOZ_EMBED_NO_SECURITY;
      //g_print("MINGTK_MOZ_EMBED_NO_SECURITY\n");
      break;
    ...
    case nsIWebProgressListener::STATE_IS_SECURE|
      nsIWebProgressListener::STATE_SECURE_HIGH:
      sec_mode = MINGTK_MOZ_EMBED_HIGH_SECURITY;
      //g_print("MINGTK_MOZ_EMBED_HIGH_SECURITY");
      break;
    case nsIWebProgressListener::STATE_IS_SECURE|
      nsIWebProgressListener::STATE_SECURE_MED:
      sec_mode = MINGTK_MOZ_EMBED_MEDIUM_SECURITY;
      //g_print("MINGTK_MOZ_EMBED_MEDIUM_SECURITY\n");
      break;
    case nsIWebProgressListener::STATE_IS_SECURE|
      nsIWebProgressListener::STATE_SECURE_LOW:
      sec_mode = MINGTK_MOZ_EMBED_LOW_SECURITY;
      //g_print("MINGTK_MOZ_EMBED_LOW_SECURITY\n");
      break;
    default:
      sec_mode = MINGTK_MOZ_EMBED_UNKNOWN_SECURITY;
      //g_print("MINGTK_MOZ_EMBED_UNKNOWN_SECURITY\n");
      break;
  }
  return sec_mode;
}
