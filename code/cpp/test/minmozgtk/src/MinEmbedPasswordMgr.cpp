  ...
  case 1:  // normal login
    {
      ...
      if (parentWidget)
        gtk_signal_emit(GTK_OBJECT(MINGTK_MOZ_EMBED(parentWidget)->common),
                        moz_embed_common_signals[COMMON_REMEMBER_LOGIN], &selection);
      // FIXME These values (0,1,2,3,4) need constant variable.
      if (selection == MINGTK_MOZ_EMBED_LOGIN_REMEMBER_FOR_THIS_SITE ) {
        SignonDataEntry* entry = new SignonDataEntry();
        ...
        AddSignonData(realm, entry);
        WritePasswords(mSignonFile);
      } else if (selection == MINGTK_MOZ_EMBED_LOGIN_REMEMBER_FOR_THIS_SERVER ) {
        SignonDataEntry* entry = new SignonDataEntry();
        ...
        AddSignonData(realm, entry);
        WritePasswords(mSignonFile);
      } else if (selection == MINGTK_MOZ_EMBED_LOGIN_NOT_NOW) {
        // cancel button was pressed.
      } else if (selection == MINGTK_MOZ_EMBED_LOGIN_NEVER_FOR_SITE || selection == MINGTK_MOZ_EMBED_LOGIN_NEVER_FOR_SERVER) {
        AddReject(realm);
      }
    }
    break;
