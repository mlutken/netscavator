                       ...
                       PRUint32 aForced)
{
  ...
  mDownload = (EmbedDownload *) MINGTK_MOZ_EMBED_DOWNLOAD(instance)->data;
  mDownload->parent = instance;

  rv = GetDownloadInfo(aLauncher, aContext);

  /* Retrieve MinGtkMozEmbed object from DOM Window */
  ...
  if (!mDownload->file_name_with_path) {
    gtk_moz_embed_download_do_command(MINGTK_MOZ_EMBED_DOWNLOAD(mDownload->parent),
