...
#include "mingtkmozembed_glue.cpp"
...
static void location_changed_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser );
static void title_changed_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser );
static void load_started_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser );
static void load_finished_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser );
static void net_state_change_cb ( MinGtkMozEmbed *embed, gint flags,
								  guint status, TestGtkBrowser *browser );
static void net_state_change_all_cb ( MinGtkMozEmbed *embed, const char *uri,
									  gint flags, guint status,
									  TestGtkBrowser *browser );
static void progress_change_cb ( MinGtkMozEmbed *embed, gint cur, gint max,
								 TestGtkBrowser *browser );
static void progress_change_all_cb ( MinGtkMozEmbed *embed, const char *uri,
									 gint cur, gint max,
									 TestGtkBrowser *browser );
static void link_message_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser );
static void js_status_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser );
static void new_window_cb ( MinGtkMozEmbed *embed,
							MinGtkMozEmbed **retval, guint chromemask,
							TestGtkBrowser *browser );
static void visibility_cb ( MinGtkMozEmbed *embed,
							gboolean visibility,
							TestGtkBrowser *browser );
static void destroy_brsr_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser );
static gint open_uri_cb ( MinGtkMozEmbed *embed, const char *uri,
						  TestGtkBrowser *browser );
static void size_to_cb ( MinGtkMozEmbed *embed, gint width,
						 gint height, TestGtkBrowser *browser );
static gint dom_key_down_cb ( MinGtkMozEmbed *embed, nsIDOMKeyEvent *event,
							  TestGtkBrowser *browser );
static gint dom_key_press_cb ( MinGtkMozEmbed *embed, nsIDOMKeyEvent *event,
							   TestGtkBrowser *browser );
static gint dom_key_up_cb ( MinGtkMozEmbed *embed, nsIDOMKeyEvent *event,
							TestGtkBrowser *browser );
static gint dom_mouse_down_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
								TestGtkBrowser *browser );
static gint dom_mouse_up_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
							  TestGtkBrowser *browser );
static gint dom_mouse_click_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
								 TestGtkBrowser *browser );
static gint dom_mouse_dbl_click_cb ( MinGtkMozEmbed *embed,
									 nsIDOMMouseEvent *event,
									 TestGtkBrowser *browser );
static gint dom_mouse_over_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
								TestGtkBrowser *browser );
static gint dom_mouse_out_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
							   TestGtkBrowser *browser );
static gint dom_activate_cb ( MinGtkMozEmbed *embed, nsIDOMUIEvent *event,
							  TestGtkBrowser *browser );
static gint dom_focus_in_cb ( MinGtkMozEmbed *embed, nsIDOMUIEvent *event,
							  TestGtkBrowser *browser );
static gint dom_focus_out_cb ( MinGtkMozEmbed *embed, nsIDOMUIEvent *event,
							   TestGtkBrowser *browser );

// callbacks from the singleton object
static void new_window_orphan_cb ( MinMinGtkMozEmbedSingle *embed,
								   MinGtkMozEmbed **retval, guint chromemask,
								   gpointer data );
...
static void
ml_gtk_moz_embed_set_path ( const char *aPath )
{
	printf("ML: BBBBBBBBBBBBBBBBB\n ");
	printf("ML: BBBBBBBBBBBBBBBBB\n ");
	printf("ML: BBBBBBBBBBBBBBBBB\n ");
	printf("ML: BBBBBBBBBBBBBBBBB\n ");
	printf("ML: BBBBBBBBBBBBBBBBB\n ");
...
main ( int argc, char **argv )
{
	...
	strcpy ( xpcomPath, XULRUNNER_SDK_MAIN_LIB "libxpcom.so" );
	...
	ml_gtk_moz_embed_set_path ( xpcomPath );
	...
	TestGtkBrowser *browser = new_gtk_browser ( MINMINGTK_MOZ_EMBED_FLAG_DEFAULTCHROME );
...
new_gtk_browser ( guint32 chromeMask )
{
	...
	if ( chromeMask == MINMINGTK_MOZ_EMBED_FLAG_DEFAULTCHROME )
		actualChromeMask = MINMINGTK_MOZ_EMBED_FLAG_ALLCHROME;

	if ( actualChromeMask & MINMINGTK_MOZ_EMBED_FLAG_MENUBARON )
	{
		browser->menuBarOn = TRUE;
		g_print ( "\tmenu bar\n" );
	}
	if ( actualChromeMask & MINMINGTK_MOZ_EMBED_FLAG_TOOLBARON )
	{
		browser->toolBarOn = TRUE;
		g_print ( "\ttool bar\n" );
	}
	if ( actualChromeMask & MINMINGTK_MOZ_EMBED_FLAG_LOCATIONBARON )
	{
		browser->locationBarOn = TRUE;
		g_print ( "\tlocation bar\n" );
	}
	if ( actualChromeMask & MINMINGTK_MOZ_EMBED_FLAG_STATUSBARON )
	{
	...
	gtk_moz_embed_set_chrome_mask ( MINGTK_MOZ_EMBED ( browser->mozEmbed ),
									actualChromeMask );
...
{
	gtk_moz_embed_go_back ( MINGTK_MOZ_EMBED ( browser->mozEmbed ) );
}
...
stop_clicked_cb ( GtkButton *button, TestGtkBrowser *browser )
{
	...
	gtk_moz_embed_stop_load ( MINGTK_MOZ_EMBED ( browser->mozEmbed ) );
}
...
forward_clicked_cb ( GtkButton *button, TestGtkBrowser *browser )
{
	...
	gtk_moz_embed_go_forward ( MINGTK_MOZ_EMBED ( browser->mozEmbed ) );
}
...
reload_clicked_cb ( GtkButton *button, TestGtkBrowser *browser )
{
	...
	gtk_moz_embed_reload ( MINGTK_MOZ_EMBED ( browser->mozEmbed ),
						   ( state & GDK_SHIFT_MASK ) ?
						   MINMINGTK_MOZ_EMBED_FLAG_RELOADBYPASSCACHE :
						   MINMINGTK_MOZ_EMBED_FLAG_RELOADNORMAL );
}
...
stream_clicked_cb ( GtkButton   *button, TestGtkBrowser *browser )
{
	...
	gtk_moz_embed_open_stream ( MINGTK_MOZ_EMBED ( browser->mozEmbed ),
								"file://", "text/html" );
	gtk_moz_embed_append_data ( MINGTK_MOZ_EMBED ( browser->mozEmbed ),
								data, strlen ( data ) );
	gtk_moz_embed_append_data ( MINGTK_MOZ_EMBED ( browser->mozEmbed ),
								data2, strlen ( data2 ) );
	gtk_moz_embed_close_stream ( MINGTK_MOZ_EMBED ( browser->mozEmbed ) );
}
...
url_activate_cb ( GtkEditable *widget, TestGtkBrowser *browser )
{
	...
	gtk_moz_embed_load_url ( MINGTK_MOZ_EMBED ( browser->mozEmbed ), text );
	g_free ( text );
}
...
menu_open_new_cb ( GtkMenuItem *menuitem, TestGtkBrowser *browser )
{
	...
	TestGtkBrowser *newBrowser =
		new_gtk_browser ( MINMINGTK_MOZ_EMBED_FLAG_DEFAULTCHROME );
...
void
location_changed_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser )
{
	char *newLocation;
	int   newPosition = 0;
...
void
title_changed_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser )
{
	char *newTitle;
...
void
load_started_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser )
{
...
void
load_finished_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser )
{
...
void
net_state_change_cb ( MinGtkMozEmbed *embed, gint flags, guint status,
					  TestGtkBrowser *browser )
{
	g_print ( "net_state_change_cb %d\n", flags );
	if ( flags & MINMINGTK_MOZ_EMBED_FLAG_IS_REQUEST )
	{
		if ( flags & MINMINGTK_MOZ_EMBED_FLAG_REDIRECTING )
			browser->statusMessage = "Redirecting to site...";
		else if ( flags & MINMINGTK_MOZ_EMBED_FLAG_TRANSFERRING )
			browser->statusMessage = "Transferring data from site...";
		else if ( flags & MINMINGTK_MOZ_EMBED_FLAG_NEGOTIATING )
			browser->statusMessage = "Waiting for authorization...";
	}

	if ( status == MINMINGTK_MOZ_EMBED_STATUS_FAILED_DNS )
		browser->statusMessage = "Site not found.";
	else if ( status == MINMINGTK_MOZ_EMBED_STATUS_FAILED_CONNECT )
		browser->statusMessage = "Failed to connect to site.";
	else if ( status == MINMINGTK_MOZ_EMBED_STATUS_FAILED_TIMEOUT )
		browser->statusMessage = "Failed due to connection timeout.";
	else if ( status == MINMINGTK_MOZ_EMBED_STATUS_FAILED_USERCANCELED )
		browser->statusMessage = "User canceled connecting to site.";

	if ( flags & MINMINGTK_MOZ_EMBED_FLAG_IS_DOCUMENT )
	{
		if ( flags & MINMINGTK_MOZ_EMBED_FLAG_START )
			browser->statusMessage = "Loading site...";
		else if ( flags & MINMINGTK_MOZ_EMBED_FLAG_STOP )
...
void net_state_change_all_cb ( MinGtkMozEmbed *embed, const char *uri,
							   gint flags, guint status,
							   TestGtkBrowser *browser )
{
...
void progress_change_cb ( MinGtkMozEmbed *embed, gint cur, gint max,
						  TestGtkBrowser *browser )
{
...
void progress_change_all_cb ( MinGtkMozEmbed *embed, const char *uri,
							  gint cur, gint max,
							  TestGtkBrowser *browser )
{
...
void
link_message_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser )
{
	char *message;
...
void
js_status_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser )
{
	char *message;
...
void
new_window_cb ( MinGtkMozEmbed *embed, MinGtkMozEmbed **newEmbed, guint chromemask, TestGtkBrowser *browser )
{
	...
	gtk_widget_set_usize ( newBrowser->mozEmbed, 400, 400 );
	*newEmbed = MINGTK_MOZ_EMBED ( newBrowser->mozEmbed );
	g_print ( "new browser is %p\n", ( void * ) *newEmbed );
}

void
visibility_cb ( MinGtkMozEmbed *embed, gboolean visibility, TestGtkBrowser *browser )
{
...
void
destroy_brsr_cb ( MinGtkMozEmbed *embed, TestGtkBrowser *browser )
{
...
gint
open_uri_cb ( MinGtkMozEmbed *embed, const char *uri, TestGtkBrowser *browser )
{
...
void
size_to_cb ( MinGtkMozEmbed *embed, gint width, gint height,
			 TestGtkBrowser *browser )
{
...
gint dom_key_down_cb ( MinGtkMozEmbed *embed, nsIDOMKeyEvent *event,
					   TestGtkBrowser *browser )
{
	PRUint32 keyCode = 0;
...
gint dom_key_press_cb ( MinGtkMozEmbed *embed, nsIDOMKeyEvent *event,
						TestGtkBrowser *browser )
{
	PRUint32 keyCode = 0;
...
gint dom_key_up_cb ( MinGtkMozEmbed *embed, nsIDOMKeyEvent *event,
					 TestGtkBrowser *browser )
{
	PRUint32 keyCode = 0;
...
gint dom_mouse_down_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
						 TestGtkBrowser *browser )
{
...
gint dom_mouse_up_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
					   TestGtkBrowser *browser )
{
...
gint dom_mouse_click_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
						  TestGtkBrowser *browser )
{
...
gint dom_mouse_dbl_click_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
							  TestGtkBrowser *browser )
{
...
gint dom_mouse_over_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
						 TestGtkBrowser *browser )
{
...
gint dom_mouse_out_cb ( MinGtkMozEmbed *embed, nsIDOMMouseEvent *event,
						TestGtkBrowser *browser )
{
...
gint dom_activate_cb ( MinGtkMozEmbed *embed, nsIDOMUIEvent *event,
					   TestGtkBrowser *browser )
{
...
gint dom_focus_in_cb ( MinGtkMozEmbed *embed, nsIDOMUIEvent *event,
					   TestGtkBrowser *browser )
{
...
gint dom_focus_out_cb ( MinGtkMozEmbed *embed, nsIDOMUIEvent *event,
						TestGtkBrowser *browser )
{
...
void new_window_orphan_cb ( MinMinGtkMozEmbedSingle *embed,
							MinGtkMozEmbed **retval, guint chromemask,
							gpointer data )
{
	...
	TestGtkBrowser *newBrowser = new_gtk_browser ( chromemask );
	*retval = MINGTK_MOZ_EMBED ( newBrowser->mozEmbed );
	g_print ( "new browser is %p\n", ( void * ) *retval );
}
...
update_nav_buttons ( TestGtkBrowser *browser )
{
	...
	can_go_back = gtk_moz_embed_can_go_back ( MINGTK_MOZ_EMBED ( browser->mozEmbed ) );
	can_go_forward = gtk_moz_embed_can_go_forward ( MINGTK_MOZ_EMBED ( browser->mozEmbed ) );
	if ( can_go_back )
