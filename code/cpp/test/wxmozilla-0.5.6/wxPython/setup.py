...
if BUILD_MOZILLA:
    ...
    mozroot = ""
    if os.environ.has_key("WXMOZROOT"):
        mozroot = os.environ["WXMOZROOT"]
        if not os.path.exists(mozroot):
            mozroot = location + "/mozilla"
            print mozroot
        mozincludes = []
        ...
        ext = Extension('mozillac', [location + "/src/wxMozillaBrowser.cpp", 
                                    location + "/src/wxMozillaBrowserChrome.cpp",
                                    location + "/src/wxMozillaEvents.cpp",
                                    location + "/src/wxMozillaWindow.cpp",
                                    location + "/src/helpers.cpp"] + swig_sources,
                        include_dirs = [mozroot, location + "/src", os.path.join(mozroot, "include"), os.path.join(mozroot,"src")] 
    									+ mozincludes
									+ includes,
                    define_macros = defines,
    
                        library_dirs = [mozroot + "/lib"] + libdirs,
                        libraries = ["embed_base_s", "xpcom", "nspr4", "profdirserviceprovider_s"] + libs,
        ...
        msg("WARNING: Mozilla extension not built - set WXMOZROOT to your mozilla tree's /dist folder and rebuild.")
