...
#define ADVANCED	1	// Set to 0 to compile simple example
...
main ( int argc, char **argv )
{
	...
	printf ( "ML: GRE_HOME 1: %s\n", xpcomPath );
////	strcpy ( xpcomPath, XULRUNNER_SDK_BASE "/xulrunner/libxpcom.so" );
	strcpy ( xpcomPath, XULRUNNER_SDK_MAIN_LIB "libxpcom.so" );
//   strcpy( xpcomPath, XULRUNNER_SDK_BASE "/xulrunner/xpcom.dll" );
	printf ( "ML: GRE_HOME 1: %s\n", xpcomPath );
	printf ( "ML: XULRUNNER_SDK_MAIN_LIB: '%s'\n", XULRUNNER_SDK_MAIN_LIB );
//     if (NS_FAILED(rv)) {
...
//     }
	...
	if ( NS_FAILED ( rv ) )
	{
		fprintf ( stderr, "Couldn't find GTKMozEmbed internal symbols." );
		return 1;
	}
                          ...
                          TestGtkBrowser *browser )
{
	...
	g_print ( "ML: 1 dom_mouse_click_cb: %p\n", embed );
	...
	if ( !domWindow )
	{
		NS_WARNING ( "ML: no dom window in content finished loading\n" );
		return NS_ERROR_NOT_INITIALIZED;
	}
	
		
//	ex_nsIDOMWindow_Name ( domWindow );	
//	ex_nsEmbedString();
	...
	if ( !domDocument )
	{
		NS_WARNING ( "ML: No DOM Document in content finished loading\n" );
		return NS_ERROR_NOT_INITIALIZED;
	}

	//ex_nsIDOMDocument_Intro ( domDocument );
	//ex_FormGet ( domDocument );

	g_print ( "ML: 2 dom_mouse_click_cb: %p\n", embed );

	//ex_nsCache();
	//ex_javascript1(webBrowser);

	g_print ( "ML: 3 dom_mouse_click_cb\n" );
	PRUint16 button;
...
#include <string>
#include <stdio.h>
int main ( int AntalArgumenter, char **Argumenter )
{
	int Nummer = 12;
	std::string Navn = "Bent";
	
	printf ( "Antal program argumenter: %d\n", AntalArgumenter ); 
	
	if ( AntalArgumenter == 2 ) {
		Navn = 	Argumenter[1];	
	}
	
	printf ( "Hej %s Nummer er: %d.\n", Navn.c_str(), Nummer );
}
