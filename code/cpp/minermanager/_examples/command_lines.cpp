...
static
bp::child
start_it ( const Command_Line& cl )
{
	std::cout << cl;
	...
	l.set_stdout_behavior ( bp::inherit_stream );
	l.set_stderr_behavior ( bp::inherit_stream );
	bp::child cp = l.start ( cl );
	std::cout << "Hejsa" << std::endl;
	return cp;
}

int
command_lines_example ( int argc, char* argv[] )
{
	//
	...
	// allowed.
	//
	bp::command_line cl3 ( "/usr/bin/test" );
	...
	// properly.
	//
	bp::command_line cl4 ( "/usr/bin/test", "[" );
//    cl4.argument("foo").argument("=").argument("foo").argument("]");
	cl4.argument ( "foo" ).argument ( "=" ).argument ( "foo" );
	run_it ( cl4 );
